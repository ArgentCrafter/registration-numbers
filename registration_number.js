...
function createList2() {
  ...
  const dataElem = document.getElementById('regList2');
  const dataHTML = template({ reg: capKeys });
...
addBtn.addEventListener('click', () => {
  const gotRegList = reg.getRegList();
  const regListKeys = Object.keys(gotRegList);
  let flagRegFound = false;

  for (let i = 0; i < regListKeys.length; i++) {
...
addBtn2.addEventListener('click', () => {
  const gotRegList = reg.getRegList2();
  const regListKeys = Object.keys(gotRegList);
  let flagRegFound = false;
...
showBtn2.addEventListener('click', () => {
  ...
  if (checkedTown) {
    ...
    const template = Handlebars.compile(templateSource);
    const dataElem = document.getElementById('regList2');
    const dataHTML = template({ reg: filteredArray });
    dataElem.innerHTML = dataHTML;
  } else {
    feedbackElem2.style.color = 'red';
